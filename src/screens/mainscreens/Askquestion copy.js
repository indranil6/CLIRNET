  ...
  runServiceForLiveSession(startTime, endTime) {
    ...
    if (startTimeDif >= 0 && endTimeDif <= 0) {
      ...
      context.setState({ isLiveStart: true }, () =>
        console.log("running live start state changed")
      );
    } else {
      ...
      myCounter = new Countdown({
        ...
        onUpdateStatus: function (sec) {
          ...
          if(sec<=SESSION_BEGIN_TIME){  
            context.setState({ isLiveStart: true }, () =>
            console.log("live start state changed")
          );
          }
        },
      ...
      myCounter = new Countdown({
        ...
        onCounterEnd: function () {
          ...
          context.setState({ isLiveStart: false }, () =>
            console.log("live end state changed")
          );
        },
      });
      myCounter.start();
    ...
    fetch(
      ...
      .then((responseJson) => {
        if (responseJson.status_code == "401") {
          ...
          let startDateTime = "2021-08-10 15:14:00";
          let endDateTime = "2021-08-31 15:20:30";
