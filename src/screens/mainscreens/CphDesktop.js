...
class CphDesktop extends React.Component {
  constructor(props) {
    console.log(props);
    super(props);
    this.id = 10;
    ...
    this.fetchArchiveData(10, arcDataFetchLimit);
  ...
  fetchArchiveData(id, fetchForm) {
    console.log("===>>>id & fetch form", id, fetchForm);
    fetch(
      ...
      .then((response) => response.json())
      .then((responseJson) => {
        console.log("===>>response archieve", responseJson);
        let status_code = responseJson.status_code;
        if (status_code == 200) {
  ...
  componentDidMount() {
    ...
    $(".li_covid").attr("id", "grandround_cal");
    ...
    fetch(url + "gr/detail?type_id=" + 10 + "&source=" + extrautm + "", {
      ...
      .then((responseJson) => {
        if (responseJson.status_code == "200") {
          this.parseResponseData(responseJson);
            ...
            <div className="full_width radius-6 mblPllsSrvsDrwBox">
              {val.specialities_name == null ? null : (
                <div className="colorBlack font_12px font400 radius-6 mblMedWikiSpeacality">
                  ...
                  {this.popover_view_spec(val.specialities_name)}
                </div>
              )}
  ...
  parseResponseData(response) {
    if (response) {
      isReadyCounter++;
      response.data.map((rData) => {
        if (rData.media_type === "video") {
          VideoJson.type_id = rData.type_id;
          if (rData.question) {
            VideoJson.name = rData.title;
          } else {
      ...
      console.log("comming for parse" + JSON.stringify(VideoJson));
      this.setState({ display: !this.state.display });
    }
  }
  ...
  render() {
    return (
      ...
      >
              ...
              <div className="container mycontainer">
                <div className="row">
                  <div className="full_width">
                    ...
                    {/* live */}
                    <div
                      className="full_width dskBody dskCphLive"
                      style={!isLive ? { display: "none " } : null}
                    >
                      <div className="row">
                        <div className="col-12 dskLeft">
                          {this.state.data_main.map((r, indexii) => (
                            <section className="full_width text-left dskGrLeftMain">
                              <div className="full_width dskGrLeftTop"></div>
                              <div className="clearfix"></div>
                              <div className="full_width dskCphTtl">
                                <h1 className="font_20px font500 colorBlack">
                                  {r.title}
                                </h1>
                                <a
                                  href="javascript:void(0)"
                                  className="dskCphTtlShare"
                                  onClick={() => this.share_url(r)}
                                >
                              ...
                              {/* <div className="full_width dskGrLeftVideoSharePnl">
											<div className="dskGrLeftVideoSharePnlLeft">
												<a href="javascript:void(0)" className={ r.myrating == true ? 'active' : ''} >
													...
													<img src={likeIcon} alt="Like" onClick={() => this.onLikeBtnPress(r.type_id, r.type, indexii)} className="translate_both dskGrLeftShareImg" />
													<img src={likeIconActive} alt="Like" onClick={() => this.onLikeBtnPress(r.type_id, r.type, indexii)} className="translate_both dskGrLeftShareImgActive" />
                          ...
                          {this.state.data_archive.length > 0 ? (
                            <section className="full_width text-left dskGrMoreList dskgrMedwikiList">
                              {/* <h4 className="font_24px font400 colorBlack dskGrMoreTtl">Archive's <span className="font_16px">({this.state.data_medwiki.length})</span></h4> */}
                              {/* <div className="clearfix"></div> */}
                              <Masonry
        ...
        <div className="font_10px font600 cphFloating_bttn">
          <img src={realTalk} />
        </div>
        {this.state.doctors.length != 0 ? (
          <Modal
