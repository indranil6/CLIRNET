...
class AutoauthContinue extends React.Component { 
  constructor(props) {
    super(props);
    this.state = {
    ...
    user_action['page_name'] = "AutoauthContinue"; 
  ...
  recordUserActivity = (action)=>{
    ...
    // const diff = user_action.start_time - endTime;
    // const sec = diff / 1000 % 60; 
    // console.log("start_time:\n"+user_action.start_time+
    // "\nend_time:\n"+endTime+
    // "\nduration:\n"+sec+
    // "\ndif:\n"+this.dateDiffToString(user_action.start_time,endTime))  
  }
...
dateDiffToString(start, end) {
  ...
  componentDidMount() {
    ...
    window.addEventListener('beforeunload', (ev) => {
      ...
      // this.reportTrackingData("browser_close");
  ...
  onDoneBtnClick() {
    ...
    if (!this.state.terms_checkbox) {
      this.recordUserActivity("Validation Error On Done click: Please Accept Our Terms and Conditions ");
      this.setState({ 'err_msg': "Please Accept Our Terms and Conditions" });
    }   
    else if (this.state.first_name == '') {
      ...
      this.setState({ 'err_msg': "Please enter full name " });
      this.slideModal(1);
    }else if (!nameFormat.test(temp.state.first_name)) {
      this.recordUserActivity("Validation Error On Done Button Click : Please enter Valid Name");
      this.setState({ 'err_msg': "Please enter Valid Name" });
      this.slideModal(1);
    }else if (this.state.email == '' && this.state.phone_no == '') {
      ...
      this.recordUserActivity("Validation Error On Done Button Click : Enter a Valid Phone Number");
      this.setState({ 'err_msg': "Enter a Valid Phone Number" });
      this.slideModal(1);
    }
  ...
  onInputChange = input => (val) => {
    this.recordUserActivity("Changeing Input of:"+input);
    this.setState({ "err_msg": '' })
  ...
  renderContent(type) {
    // this.recordUserActivity("render"+type);
    switch (type) {
      case "medwiki":
        return (
          this.renderMedwiki()
              ...
              <div className="medWikiLeft">
                <div className="full_width radius-6 feedRow">
                  ...
                  <div class="full_width font_16px feedRow_ans">
                    <p>{ReactHtmlParser(content_view.answer)}</p>
                  </div>
                </div>
              </div>
      ...
      <div className="full_width dskScreen signupScreen">
        ...
        <Modal id="root-modal" backdrop={'static'} keyboard={false} className="in ssnCancelPop" centered="true" animation="slide" show={this.state.showSignUpModal} onHide={() => { this.setState({ "showSignUpModal": false }); }}>
          ...
          {(this.state.alreadyRegistered) ?null:
              <a href="javascript:void(0)" className="radius-40 font500 btnBlue cmnBtn btnCmnSmall" variant="secondary" onClick={() => {this.onContinueClick()}}>
                Continue
              </a>}
            {/* {(this.state.screen_no == 1 && !this.state.alreadyRegistered) ?
             ...
             {(this.state.screen_no == 1 && this.state.alreadyRegistered) ?
              <>
                <a href="javascript:void(0)" className="radius-40 font500 btnBlue cmnBtn btnCmnSmall" variant="secondary" onClick={() => {this.redirectToLogin()}}>
                  Sign In
                </a>
              </> : null
            }
  ...
  onContinueClick =()=>{
    let temp = this
    this.recordUserActivity("Clicked On Continue Btn");
    this.setState({ 'err_msg': "" });
    this.setState({ 'err_msg': "" });
    if (this.state.first_name == '') {
      this.recordUserActivity("Validation error on Continue button click: Please enter full name");
      this.setState({ 'err_msg': "Please enter full name " });
    }else if (!nameFormat.test(temp.state.first_name)) {
      this.recordUserActivity("Validation Error On Continue Button Click : Please enter Valid Name");
      this.setState({ 'err_msg': "Please enter Valid Name" });
      this.slideModal(1);
    }else if (this.state.email == '' && this.state.phone_no == '') {
      this.recordUserActivity("Validation Error On Continue Button Click : Please enter email or phone no");
      this.setState({ 'err_msg': "Please enter email or phone no" });
      this.slideModal(1);

    }else if (this.state.phone_no !== '' && this.state.isd_no == undefined) {
      this.recordUserActivity("Validation error on Continue button click: Please select country code");
      this.setState({ 'err_msg': "Please select country code" });
    }
    else if (!this.isEmail(this.state.email) && this.state.phone_no && !this.state.phone_no.match(phonenoFormat)) { 
      this.recordUserActivity("Validation error on Continue button click : Please Enter mobile no in correct format");
      this.setState({ 'err_msg': "Please Enter mobile no in correct format" });
    }
    else if (!this.state.phone_no.match(phonenoFormat) && this.state.email && !this.isEmail(this.state.email)) { 
      this.recordUserActivity("Validation error on Continue button click : Please Enter email in correct format");
      this.setState({ 'err_msg': "Please Enter email in correct format" });
    }
    else {
      // alert("signup success")
      this.skipSignUpCLick(); 
    }
  }

  skipSignUpCLick() { 
    this.recordUserActivity("Sign Up on Continue click - autologin/continueSignup -API"); 
    let first_name = this.state.first_name;
    let last_name = this.state.last_name;
    let middle_name = this.state.middle_name;
    let email = this.state.email;
    ...
    fetch(url + 'autologin/continueSignup', {
      method: 'POST',
      headers: {
        'Authorization': temp_token,
        'version': 'rjsw 1.1.1'
...
export default withRouter(AutoauthContinue);