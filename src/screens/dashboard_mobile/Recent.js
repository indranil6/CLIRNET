...
class Recent extends React.Component {
  ...
  fetchData() {
    const recentSection = document.querySelector("#recent-section");
    // let recent_list_length = recent_list_data.length + parseInt(1)
  ...
  getRecentData(fetch_from) {
    ...
    fetch(url + 'dashboard/trending?from=' + fetch_from + '&to=5&type=all', {
      ...
      if(status_code == 200){
        ...
        //console.log("after permis"+recent_api_call_permission) 
        this.refresh()
      }
    }).catch((error) => {
      recent_api_call_permission = true;
  ...
  render() {
    return (
      <section className="full_width text-left mblRecent" id="recent-section">
        ...
        {fetch_counter!=0?<Loader className="loader_cmn" type="ThreeDots" color="#355ed3" height={40} width={40} visible={this.state.loader_status} />:null} 
      </section>
    );
  }
}
